//
// qwt_scale_widget.cpp de Qwt 5.2.0 contenant un correctif dans
// QwtScaleWidget::drawTitle (problème d'écriture dans une QImage, marche bien
// dans les autres sorties).
// Modifications effectuées par C. PIGNEROL
//
    ...
    QwtScaleDraw::Alignment align, const QRect &rect) const
{
	...
	if ((0. == angle) || (0 == image))	// Cas standard
	{
	...
	{	
		// CP : Bogue probable de Qwt 5.2.0 lors d'écritures dans une QImage.
		// Le texte est écrit caractères verticaux, de bas en haut, caractères
		// collés les uns aux autres ce qui rend difficile la lecture.
		// Les caractères ASCII passent bien. Ca n'est pas le cas des caractères
		// grecques ISO.

		// Ce contournement de bogue peu élégant écrit de manière horizontale le
		// texte dans une image de sa zone de destination ayant préalablement 
		// subit la rotation inverse de celle à faire, puis écrit le texte de
		// manière horizontale (ça marche), effectue la rotation inverse de
		// l'image et copie enfin cette image avec le texte correctement orienté
		// dans l'image demandée.

		// Attention : r.width ( ) et r.height ( ) sont inversés.
		QImage	textImage	=
			image->copy (r.x ( ), r.y ( ) - r.width ( ), r.height ( ),
			r.width ( ));
		QMatrix		matrix, invMatrix;
		invMatrix.rotate (-angle);
		matrix.rotate (angle);
		textImage	= textImage.transformed (
										invMatrix, Qt::SmoothTransformation);
		QPainter	textPainter (&textImage);
		QwtText	title	= d_data->title;
		title.setRenderFlags (flags);
		title.draw (&textPainter, QRect(0, 0, r.width ( ), r.height ( )));
		textPainter.end ( );
		textImage	= textImage.transformed (matrix, Qt::SmoothTransformation);
		painter->drawImage (r.x ( ), r.y ( ) - r.width ( ), textImage);
	}	// else if ((0. == angle) || (0 == image))
